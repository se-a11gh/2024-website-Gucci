...
/* Отправка в корзину */
  $(function(){
    ...
    if(e.target.dataset.btn === '1'){    
      $.ajax({
        url: 'http://localhost/Portfolio/Gen_next/GucciX/DB/content.php',
        type: "POST",
        data: {          
          name1: data[nubrId]
        },
...
/* Фильтр(выпадающий список) (потом оптимизировать)*/
let adress_list = document.querySelector('.filter_list');
if(adress_list){
...
closeAdr.addEventListener('click', (e)=>{
    bron.style.height = "auto";
    adress_list.addEventListener('mouseleave', ()=>{
    bron.style.height = "0.0002%";
    });
});
}
